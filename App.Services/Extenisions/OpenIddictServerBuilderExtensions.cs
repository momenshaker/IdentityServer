...
{
    /// <summary>
    /// Provides extension methods for configuring OpenIddict server with certificates.
    /// </summary>
    public static class OpenIddictServerBuilderExtensions
    {
        /// <summary>
        /// Adds encryption and signing certificates to the OpenIddict server based on configuration.
        /// </summary>
        /// <param name="builder">The OpenIddict server builder to add certificates to.</param>
        /// <param name="configuration">The application configuration that contains certificate settings.</param>
        ...
        {
            // Bind the certificate settings from the configuration
            var options = new OpenIdServerOptions();
            configuration.GetSection("OpenIdServer").Bind(options);

            // Add encryption certificate based on the store type
            AddEncryptionCertificate(builder, options.EncryptionCertificate);

            // Add signing certificate based on the store type
        ...
        {
            storeName = storeName ?? "Certificates"; // Default folder name if not specified
